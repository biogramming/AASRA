...
##command line example: AASRA -f -p 4 -i sample.fa -l CCCCC -r GGGGG -b sample.fa

import argparse
import re
import sys

parser = argparse.ArgumentParser(prog='AASRA', usage='%(prog)s [-h] -f -p THREAD -i INPUT -l LEFTANCHORSEQ -r RIGHTANCHORSEQ -b BOWTIE2INDEX', description='Example: AASRA -f -p 4 -i sample.fa -l CCCCC -r GGGGG -b sample.fa')
parser.add_argument('-p','--thread', help='Input the number of threads')
parser.add_argument('-f','--fasta', help='Input sample is fasta file')
...
parser.add_argument('-b','--bowtie2index',help='Input pre-built bowtie2 index.', required=True)
