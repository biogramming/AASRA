...
##command line example: AASRA -p 4 -i sample.fastq -l CCCCC -r GGGGG -b anchored_index.fa

import argparse
import re
import sys

parser = argparse.ArgumentParser(prog='AASRA', usage='%(prog)s [-h] [-f] -p THREAD -i INPUT -l LEFTANCHORSEQ -r RIGHTANCHORSEQ -b BOWTIE2INDEX', description='Example: AASRA -p 4 -i sample.fastq -l CCCCC -r GGGGG -b anchored_index.fa')
